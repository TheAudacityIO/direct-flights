...
- [ ] First metered overlay refresh: dispatch `data-refresh.yml`, read its quota telemetry (measured unit cost per call, units remaining), then tune `ADB_MAX_AIRPORTS` to whatever the 400-unit free budget actually allows. The script self-limits via live quota headers, floor at 100 units.

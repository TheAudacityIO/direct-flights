...
Resume queue for a fresh session; work top to bottom. State below is accurate as of 2026-09-11, commit 5a32229 (rename, purge, hero, SEO pages and the Grok banner removal shipped and verified live on flydirectfrom.com; see Done). Items 2 and 3 are HUMAN GATES (Google accounts); 4 is code, deferred.
...
3. [ ] Ads wiring: waits for Miguel's AdSense `ca-pub` id and data-tier/timing answers; then ads.txt, adsbygoogle, certified CMP (Consent Mode v2), rewarded-unlock loop per the Next section. Prerequisites now live: privacy page, canonical host, crawlable airport pages, sitemap, robots.
4. [ ] Operating vs marketing carrier in the overlay: AeroDataBox `operators` names the operating carrier, so CAG-FCO and CAG-LIN show "Fly Air41 Airways" (ITA Airways' wet-lease operator on the Sardinia PSO routes) instead of ITA. Users search for the brand they book. Check whether the daily-routes payload carries a marketing/codeshare field; if not, a small operator->brand map in the overlay (Air41 -> ITA Airways for CAG/OLB/AHO PSO routes) is the honest fix, labelled "operated by".
5. [ ] /from index weight: 2936 links render to ~930 KB of HTML (loader data is dehydrated into the page on top of the markup). Fine behind Cloudflare compression for a crawl index, wrong for humans; split per country (`/from/country/{slug}`) if Search Console shows it crawled slowly or if it ever gets real visits.

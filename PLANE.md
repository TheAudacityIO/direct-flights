...
- [ ] Data upgrade (data-source assessment 2026-09-10, fast path corrected same day after live pricing verification): the OpenFlights dump is ~2014-stale and the observed-route union (`src/lib/flights/observed.ts`) is wired but UNFED. Fix: **AeroDataBox** via RapidAPI or API.Market, self-serve, paid tiers from $7.50/mo (free tier too small for a full ~3,000-airport rebuild), real data from the first call so the CAG–DUB Ryanair canary is testable day one. HUMAN GATE (~10 min): Miguel subscribes, then stores the key: `vault kv put secret/projects/direct-flights aerodatabox_api_key=…`; the integration reads exactly that field name. Then: overlay in `build-openflights.ts` feeding `buildObservedDataset`, size units per full refresh, cadence to fit the tier (weekly top airports, monthly long tail). ⚠ Prerequisite for the rewarded premium features (schedule/frequency depth needs dated data).

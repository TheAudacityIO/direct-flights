...
Resume queue for a fresh session; work top to bottom. State below is accurate as of 2026-09-11, commit 785328b (rename, purge, hero, SEO pages and the Grok banner removal shipped and verified live on flydirectfrom.com; see Done). Items 2 and 3 are HUMAN GATES (Google accounts); 4 waits on 3; 5 is code, conditional.
...
3. [ ] AdSense site verification: Miguel added flydirectfrom.com in AdSense (2026-09-11, status "Requires review") and the file-method Verify failed because no verification file was ever committed (`public/` has no `google*.html`). Needs the snippet from the AdSense screen: the meta-tag or code-snippet method is preferred since either yields the `ca-pub` id; the file method works too (static file under `public/`, deploys in ~2 min). Then "Request review".
